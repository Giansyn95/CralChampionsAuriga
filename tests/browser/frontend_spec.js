...
for (const torneo of active) {
  test(`frontend torneo ${torneo.id} carica dati e navigazione`, async ({ page }) => {
    ...
    // Anche il nome squadra in Classifica squadre è un link di navigazione primario:
    // un tap reale deve aprire subito la scheda, anche quando il nome è troncato
    // (es. su mobile) e il sistema di tooltip per il testo troncato prova a
    // intercettare il tap. Regression test per il bug del secondo tap che non
    // apriva mai la scheda squadra.
    const rankedTeamName = await page.evaluate(() => {
      const row=(state.rows['classifica_squadre']||[])[0]||null;
      return row ? firstPresent(row,[['squadra','team','nome squadra']]) : '';
    });
    if (rankedTeamName) {
      await page.evaluate(() => showTab('classifiche', false, { skipRoute:true }));
      const teamLink = page.locator('#classifiche .team-link').filter({ hasText: rankedTeamName }).first();
      await expect(teamLink, `Link squadra non trovato per ${rankedTeamName}`).toBeVisible();
      await teamLink.click();
      await expect(page.locator('.team-profile-view'), `La scheda squadra non si è aperta per ${rankedTeamName}`).toBeVisible();
