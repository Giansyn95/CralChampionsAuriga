...
(() => {
  ...
  function adminExtension() {
    ...
    function proStyle() {
      ...
      style.textContent = `
        ...
        .pro-nav-group{font-size:10px;font-weight:900;letter-spacing:.08em;text-transform:uppercase;color:var(--muted,#667085);padding:10px 12px 4px}
        .pro-mobile-nav-wrap{display:none}
        .pro-mobile-nav{width:100%;border:1px solid var(--line,#dbe5f2);border-radius:12px;background:var(--panel,#fff);color:var(--ink,#142033);padding:11px 12px;font-weight:800}
        @media(max-width:760px){
          .pro-history-row{grid-template-columns:1fr}.pro-table{font-size:12px}
          .sidebar{display:block!important;overflow:visible!important;padding:8px!important}
          .sidebar>.nav-btn,.sidebar>.pro-nav-sep,.sidebar>.pro-nav-group{display:none!important}
          .pro-mobile-nav-wrap{display:block}
          .pro-actions>.btn{flex:1 1 100%}
          .pro-kpi{min-width:calc(50% - 5px);flex:1 1 calc(50% - 5px)}
        }
      `;
    ...
    refreshSidebar = function() {
      ...
      groups.forEach(([groupLabel, items], gi) => {
        if (gi) side.appendChild(el('div','pro-nav-sep'));
        side.appendChild(el('div','pro-nav-group',groupLabel));
        items.forEach(([id,icon,label]) => {
          const b = el('button',`nav-btn ${state.active===id?'active':''}`);
          b.type='button';
          b.appendChild(document.createTextNode(`${icon} ${label}`));
          if (id==='publish' && state.pending.size) b.appendChild(el('span','pending-pill',state.pending.size));
          b.addEventListener('click',()=>{state.active=id;render()});
          side.appendChild(b);
        });
      });
      const mobileWrap = el('div','pro-mobile-nav-wrap');
      const mobile = el('select','pro-mobile-nav');
      groups.forEach(([groupLabel, items]) => {
        const og = document.createElement('optgroup');
        og.label = groupLabel;
        items.forEach(([id,icon,label]) => {
          const o = document.createElement('option');
          o.value=id; o.textContent=`${icon} ${label}`; o.selected=state.active===id;
          og.appendChild(o);
        });
        mobile.appendChild(og);
      });
      mobile.addEventListener('change',()=>{state.active=mobile.value;render()});
      mobileWrap.appendChild(mobile);
      side.prepend(mobileWrap);
    };
    ...
    createTournament = async function(...args) {
      ...
      if (state.model && state.active === 'squadre' && /creato/i.test(state.status?.text || '')) {
        state.active='setup';
        if(makeCurrent)state.status={type:'success',text:`${state.status.text} Se esisteva un torneo corrente precedente, è stato automaticamente marcato come concluso.`};
        render();
      }
    };
    ...
    function proDetectCsv(fileName,text){
      ...
      if(proHas(h,'squadra casa','casa') && proHas(h,'squadra trasferta','trasferta') && proHas(h,'giornata')){
        const hasScoreColumns=proHas(h,'gol casa','goal casa','gc')||proHas(h,'gol trasferta','goal trasferta','gt')||proHas(h,'risultato');
        const hasFilledScore=hasScoreColumns && objects.some(o=>{
          const home=String(proField(o,['gol casa','goal casa','gc'])??'').trim();
          const away=String(proField(o,['gol trasferta','goal trasferta','gt'])??'').trim();
          const result=String(proField(o,['risultato','esito','score'])??'').trim();
          return home!==''||away!==''||result!=='';
        });
        const name=norm(fileName);
        const resultNameHint=/risultat|result|esit/.test(name);
        const calendarNameHint=/calendario|calendar/.test(name);
        const scores=hasFilledScore||(resultNameHint&&!calendarNameHint);
    ...
    function proRenderImport(main){
      ...
      const card=el('div','card');const drop=el('div','pro-drop');drop.appendChild(el('strong','','Seleziona uno o più CSV'));drop.appendChild(el('div','pro-muted','Supportati: rose singole/multiple, calendario, risultati, riepilogo aggregato, classifiche e config.'));
    ...
    function proGenerateCalendar(){
      ...
      state.status={type:'success',text:`Calendario generato: ${day-1} giornate, ${all.length} partite. Il file canonico sarà calendario.csv.`};render();
    }
    ...
    function proRenderSettings(main){
      main.appendChild(pageHead('Impostazioni torneo','Gestisci metadati, stato, visibilità e torneo corrente direttamente da tornei.json.'));
      ...
      const entry=proRegistryEntry();if(!entry){main.appendChild(messageBox('error','Il torneo corrente non è presente in tornei.json. Usa Verifica per diagnosticare il problema.'));return}
    ...
    async function proCommitLogo(file){
      ...
      try{
        const {base64}=await proConvertImage(file,'image/png',1400,1);
        const target=proCurrentTarget();
        const registry=parseTournamentRegistry(await getTextFile(target,'tornei.json'));
        const entry=(registry.tornei||[]).find(t=>String(t.cartella||'').replace(/^\/+|\/+$/g,'')===state.tournament);
        const logoBlob=await proGhJson(target,'/git/blobs',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({content:base64,encoding:'base64'})});
        const changes=[{path:`${state.tournament}/immagini/logo_cral.png`,sourceSha:logoBlob.sha}];
        if(entry?.corrente){registry.logo=`${state.tournament}/immagini/logo_cral.png`;changes.push({path:'tornei.json',content:JSON.stringify(registry,null,2)+'\n'})}
        const result=await createAtomicCommit(target,{baseCommitSha:state.snapshot.commitSha,changes,message:`Admin CRAL: aggiorna logo ${entry?.id||state.tournament}`});
        await proRefreshAll(`Logo aggiornato. Commit ${result.sha.slice(0,8)}.`);
    ...
    function proBuildVerifyReport(){
      ...
      const singleton=['calendario','risultati','classifica_squadre','marcatori','mvp','portieri'];singleton.forEach(kind=>{const fs=sectionFiles(model,kind);if(fs.length>1)warnings.push(`Più file attivi di tipo ${kind}: ${fs.map(f=>f.rel).join(', ')}.`)});
      ...
      const fileByRel=new Set(model.fileList.map(f=>norm(f.rel)));model.manifestEntries.forEach(rel=>{const exists=fileByRel.has(norm(rel))||(fileKind(rel)==='pagellone'&&fileByRel.has(norm(`pagelloni/${rel}`)));if(!exists)errors.push(`manifest.csv punta a un file non caricato/esistente: ${rel}.`)});
      ...
      const summaryFiles=sectionFiles(model,'riepilogo');if(summaryFiles.length>1){const daySources=new Map();model.summaryMatches.forEach(m=>{if(!m.day)return;const set=daySources.get(m.day)||new Set();set.add(m.sourceFile);daySources.set(m.day,set)});[...daySources].filter(([,s])=>s.size>1).forEach(([d,s])=>errors.push(`Giornata ${d} presente in più riepiloghi attivi: ${[...s].join(', ')}.`))}
    ...
    async function proPromotionInfo(){
      if(state.target!=='collaudo')throw new Error('Apri la sezione Promuovi dall’ambiente Collaudo.');
      const src=requireTarget('collaudo'),dst=requireTarget('produzione');
      const [sh,dh]=await Promise.all([getHead(src),getHead(dst)]);
      const [st,dt,sreg,dreg]=await Promise.all([getTree(src,sh.treeSha),getTree(dst,dh.treeSha),getTextFile(src,'tornei.json'),getTextFile(dst,'tornei.json')]);
      const sourceFiles=(st.tree||[]).filter(x=>x.type==='blob'&&x.path.startsWith(state.tournament+'/'));
      ...
      const destByPath=new Map(destFiles.map(f=>[f.path,f]));
      const added=sourceFiles.filter(f=>!destByPath.has(f.path));
      const updated=sourceFiles.filter(f=>destByPath.has(f.path)&&destByPath.get(f.path).sha!==f.sha);
      const unchanged=sourceFiles.filter(f=>destByPath.has(f.path)&&destByPath.get(f.path).sha===f.sha);
      const removed=destFiles.filter(f=>!sourceByPath.has(f.path));
      const sr=parseTournamentRegistry(sreg),dr=parseTournamentRegistry(dreg);
      const meta=(sr.tornei||[]).find(t=>String(t.cartella||'').replace(/^\/+|\/+$/g,'')===state.tournament);
      if(!meta)throw new Error('Metadati torneo non trovati in Collaudo.');
      return{src,dst,sh,dh,sourceFiles,destFiles,sr,dr,meta,added,updated,unchanged,removed}
    }
    ...
    async function proPromote(makeCurrent,confirmation){
      if(!proNoPending('Promozione in Produzione'))return;
      if(confirmation!=='PROMUOVI PRODUZIONE'){state.status={type:'error',text:'Digita esattamente PROMUOVI PRODUZIONE.'};render();return}
      try{
        renderLoading('Promozione torneo in Produzione...');
        const info=await proPromotionInfo();
        const changes=[];
        for(const f of [...info.added,...info.updated]){
          const sourceBlob=await proGhJson(info.src,`/git/blobs/${f.sha}`);
          if(sourceBlob.encoding!=='base64')throw new Error(`Encoding non supportato per ${f.path}.`);
          const copiedBlob=await proGhJson(info.dst,'/git/blobs',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({content:String(sourceBlob.content||'').replace(/\s+/g,''),encoding:'base64'})});
          changes.push({path:f.path,sourceSha:copiedBlob.sha});
        }
        info.removed.forEach(f=>changes.push({path:f.path,delete:true}));
        const reg=structuredClone(info.dr);
        const old=(reg.tornei||[]).findIndex(t=>String(t.cartella||'').replace(/^\/+|\/+$/g,'')===state.tournament);
        const meta=structuredClone(info.meta);
        if(makeCurrent){
          (reg.tornei||[]).forEach(t=>{if(t.corrente&&String(t.cartella||'').replace(/^\/+|\/+$/g,'')!==state.tournament){t.corrente=false;if(String(t.stato||'')==='in-corso')t.stato='concluso'}});
          meta.corrente=true;meta.stato='in-corso';reg.logo=`${state.tournament}/immagini/logo_cral.png`;
        }else meta.corrente=false;
        if(old>=0)reg.tornei[old]=meta;else reg.tornei.push(meta);
        changes.push({path:'tornei.json',content:JSON.stringify(reg,null,2)+'\n'});
        const result=await createAtomicCommit(info.dst,{baseCommitSha:info.dh.commitSha,changes,message:`Admin CRAL: promuovi ${meta.id||state.tournament} in produzione`});
        state.status={type:'success',text:`Promozione completata in Produzione. Aggiunti ${info.added.length}, aggiornati ${info.updated.length}, eliminati ${info.removed.length}, invariati ${info.unchanged.length}. Commit ${result.sha.slice(0,8)}.`};
        state.proPromotionPreview=null;render();
      }catch(e){state.status={type:'error',text:e.message||String(e)};render()}
    }
    function proRenderPromote(main){
      ...
      if(state.target!=='collaudo'){main.appendChild(messageBox('warning','Questa funzione parte dal Collaudo. Cambia Ambiente in alto e torna qui.'));return}if(!state.targets.produzione){
        const setupCard=el('div','card');
        setupCard.appendChild(messageBox('warning','Per promuovere un torneo, l’Admin deve conoscere anche le credenziali di Produzione nella sessione corrente. Configura una volta l’ambiente Produzione con un Personal Access Token GitHub che abbia permesso Contents: Read and write sul repository cralauriga/CralChampionsAuriga. Dopo l’accesso torna su Collaudo e riapri Promuovi.'));
        setupCard.appendChild(button('Configura Produzione','secondary',()=>{
          if(state.pending.size&&!window.confirm('Passando a Produzione verranno scartate le modifiche non pubblicate. Continuare?'))return;
          state.target='produzione';
          renderLogin('produzione','Inserisci il token GitHub autorizzato sul repository di Produzione. Dopo l’accesso, torna su Collaudo dal selettore Ambiente.');
        }));
        main.appendChild(setupCard);return
      }const card=el('div','card');card.appendChild(el('p','',`Torneo sorgente: ${state.tournament}`));card.appendChild(button('Confronta con Produzione','secondary',proPreviewPromotion));if(state.proPromotionPreview){const p=state.proPromotionPreview;card.appendChild(messageBox('info',[`File Collaudo: ${p.sourceFiles.length}.`,`Aggiunti in Produzione: ${p.added.length}.`,`Aggiornati: ${p.updated.length}.`,`Eliminati perché non più presenti in Collaudo: ${p.removed.length}.`,`Invariati: ${p.unchanged.length}.`,`La promozione esegue un mirror della cartella torneo: aggiunge, aggiorna ed elimina in un unico commit.`]));const current=input('checkbox');current.checked=!!p.meta.corrente;const lab=el('label','checkbox-row');lab.append(current,document.createTextNode(' Imposta come torneo corrente anche in Produzione'));card.appendChild(lab);const conf=input('text','');conf.placeholder='PROMUOVI PRODUZIONE';card.appendChild(fieldWrap('Conferma',conf));card.appendChild(button('Promuovi ora','gold',()=>proPromote(current.checked,conf.value)))}main.appendChild(card)
    }
    ...
    async function proRollbackTournament(commitSha,includeRegistry=false){
      ...
      if(!window.confirm(`Ripristinare ${state.tournament} al commit ${commitSha.slice(0,8)}? Verrà creato un NUOVO commit di ripristino.`))return;
      try{
        renderLoading('Preparazione rollback torneo...');
        const t=proCurrentTarget();
        const [head,oldCommit]=await Promise.all([getHead(t),proGhJson(t,`/git/commits/${commitSha}`)]);
        const [nowTree,oldTree]=await Promise.all([getTree(t,head.treeSha),getTree(t,oldCommit.tree.sha)]);
        const prefix=state.tournament+'/';
        const cur=new Map((nowTree.tree||[]).filter(x=>x.type==='blob'&&x.path.startsWith(prefix)).map(x=>[x.path,x]));
        const old=new Map((oldTree.tree||[]).filter(x=>x.type==='blob'&&x.path.startsWith(prefix)).map(x=>[x.path,x]));
        const changes=[];
        cur.forEach((v,p)=>{if(!old.has(p))changes.push({path:p,delete:true})});
        old.forEach((v,p)=>{if(!cur.has(p)||cur.get(p).sha!==v.sha)changes.push({path:p,sourceSha:v.sha})});
        if(includeRegistry){
          const oldRegBlob=(oldTree.tree||[]).find(x=>x.type==='blob'&&x.path==='tornei.json');
          if(!oldRegBlob)throw new Error('Il commit selezionato non contiene tornei.json.');
          const [currentRegistryText,oldRegistryText]=await Promise.all([getTextFile(t,'tornei.json'),getBlobText(t,oldRegBlob.sha)]);
          const currentRegistry=parseTournamentRegistry(currentRegistryText);
          const oldRegistry=parseTournamentRegistry(oldRegistryText);
          const key=x=>String(x?.cartella||'').replace(/^\/+|\/+$/g,'');
          const oldEntry=(oldRegistry.tornei||[]).find(x=>key(x)===state.tournament);
          if(!oldEntry)throw new Error('Nel commit selezionato il torneo non è presente in tornei.json.');
          const nextRegistry=structuredClone(currentRegistry);
          const idx=(nextRegistry.tornei||[]).findIndex(x=>key(x)===state.tournament);
          const restored=structuredClone(oldEntry);
          if(idx>=0){
            restored.corrente=!!nextRegistry.tornei[idx].corrente;
            nextRegistry.tornei[idx]=restored;
          }else{
            restored.corrente=false;
            nextRegistry.tornei.push(restored);
          }
          changes.push({path:'tornei.json',content:JSON.stringify(nextRegistry,null,2)+'\n'});
        }
        if(!changes.length){state.status={type:'warning',text:'Il torneo coincide già con la versione selezionata.'};render();return}
        const result=await createAtomicCommit(t,{baseCommitSha:head.commitSha,changes,message:`Admin CRAL: rollback ${state.tournament} a ${commitSha.slice(0,8)}`});
        await proRefreshAll(`Rollback completato con nuovo commit ${result.sha.slice(0,8)}.`);
      }catch(e){state.status={type:'error',text:e.message||String(e)};render()}
    }
    function proRenderHistory(main){
      ...
      const opt=el('label','checkbox-row');const include=input('checkbox');include.checked=!!state.proHistoryIncludeRegistry;include.addEventListener('change',()=>state.proHistoryIncludeRegistry=include.checked);opt.append(include,document.createTextNode(' Ripristina anche i metadati di questo torneo in tornei.json (senza sostituire il registro completo; la selezione del torneo corrente resta invariata)'));main.appendChild(opt);
  ...
  window.fetch = async function(input, init) {
    ...
    try {
      const requestHref = typeof input === 'string' ? input : (input?.href || input?.url || String(input || ''));
      const url = new URL(requestHref, location.href);
