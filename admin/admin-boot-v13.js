/*
 * CRAL Champions Admin - bootstrap logica v13
 *
...
function replaceOnce(source, needle, replacement, label) {
  ...
  if (index < 0) throw new Error(`Patch v13 non applicabile (${label}). Ricarica i file Admin dalla versione attesa.`);
  return source.slice(0, index) + replacement + source.slice(index + needle.length);
}

function replaceRegexOnce(source, regex, replacement, label) {
  if (!regex.test(source)) throw new Error(`Patch v13 non applicabile (${label}). Ricarica i file Admin dalla versione attesa.`);
  regex.lastIndex = 0;
...
async function loadSource(name) {
  ...
  url.searchParams.set('v13src', VERSION);
...
function draftKey(day){return \`cral-admin-draft-v13|${'${state.target}'}|${'${state.tournament}'}|${'${day}'}\`}
function saveLocalDayDraft()`,
...
async function boot() {
  try {
    ...
    const coreUrl = moduleUrl(patchCore(coreOriginal), 'cral-core-v13.js');
    const ghUrl = moduleUrl(ghPatched, 'cral-gh-v13.js');
    const adminUrl = moduleUrl(patchAdmin(adminOriginal, coreUrl, ghUrl), 'cral-admin-v13.js');

    await import(adminUrl);
  } catch (error) {
    console.error('[CRAL Admin v13]', error);
    ...
    if (app) {
      ...
      const title = document.createElement('h2'); title.textContent = 'Errore bootstrap Admin v13';
