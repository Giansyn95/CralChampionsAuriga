/*
 * CRAL Champions Admin - bootstrap logica v30
 *
...
function replaceOnce(source, needle, replacement, label) {
  ...
  if (index < 0) throw new Error(`Patch v30 non applicabile (${label}). Ricarica i file Admin dalla versione attesa.`);
  return source.slice(0, index) + replacement + source.slice(index + needle.length);
}

function replaceRegexOnce(source, regex, replacement, label) {
  if (!regex.test(source)) throw new Error(`Patch v30 non applicabile (${label}). Ricarica i file Admin dalla versione attesa.`);
  regex.lastIndex = 0;
...
async function loadSource(name) {
  ...
  url.searchParams.set('v30src', VERSION);
...
function setAlias(row, headers, aliases, value, fallbackHeader) {
  ...
  // 10) v30: la riga "Partita" del riepilogo deve essere autosufficiente.
  // Rigenera Marcatori/MVP dalle righe strutturate della bozza e conserva soltanto
  // le note libere (es. "Riposa: ..."). In questo modo una pubblicazione non puo'
  // impoverire Note lasciando il FE senza i marcatori interni.
  s = replaceOnce(
    s,
    "function buildSummaryRows(draft) {",
    `function summaryShortPlayerName(value) {
  const parts=String(value||'').trim().split(/\\s+/).filter(Boolean);
  if(parts.length<=1)return parts[0]||'';
  return parts.slice(1).join(' ');
}
function summaryManagedNoteParts(match) {
  const raw=String(match?.notes||'').trim();
  if(!raw)return [];
  return raw.split(/\\s*\\|\\s*/).map(x=>x.trim()).filter(Boolean).filter(part=>{
    const n=norm(part);
    return !(n.startsWith('marcatori') || n.startsWith('marcatore') || n.startsWith('mvp'));
  });
}
function summaryMatchNotes(match) {
  const details=match?.details||{};
  const scorerParts=(details.scorers||[])
    .filter(s=>!s.external && norm(s.player)!=='esterno')
    .map(s=>{
      const name=summaryShortPlayerName(s.player)||String(s.player||'').trim();
      const qty=Math.max(1,Number(s.qty||1));
      return name+(qty>1?' x'+qty:'');
    })
    .filter(Boolean);
  const parts=[];
  if(scorerParts.length)parts.push('Marcatori: '+scorerParts.join(', '));
  if(details.mvp && (details.mvp.player || details.mvp.external)) {
    const mvpName=details.mvp.external
      ? (String(details.mvp.player||'').trim() || 'Esterno')
      : (summaryShortPlayerName(details.mvp.player) || String(details.mvp.player||'').trim());
    if(mvpName)parts.push('MVP: '+mvpName);
  }
  parts.push(...summaryManagedNoteParts(match));
  return parts.join(' | ');
}
function buildSummaryRows(draft) {`,
    'note partita strutturate'
  );
  s = replaceOnce(
    s,
...
async function boot() {
  try {
    ...
    const coreUrl = moduleUrl(patchCore(coreOriginal), 'cral-core-v30.js');
    const ghUrl = moduleUrl(ghPatched, 'cral-gh-v30.js');
    const adminUrl = moduleUrl(patchAdmin(adminOriginal, coreUrl, ghUrl), 'cral-admin-v30.js');

    await import(adminUrl);
  } catch (error) {
    console.error('[CRAL Admin v30]', error);
    ...
    if (app) {
      ...
      const title = document.createElement('h2'); title.textContent = 'Errore bootstrap Admin v30';
