/*
 * CRAL Champions Admin - bootstrap logica v15
 *
...
function replaceOnce(source, needle, replacement, label) {
  ...
  if (index < 0) throw new Error(`Patch v15 non applicabile (${label}). Ricarica i file Admin dalla versione attesa.`);
  return source.slice(0, index) + replacement + source.slice(index + needle.length);
}

function replaceRegexOnce(source, regex, replacement, label) {
  if (!regex.test(source)) throw new Error(`Patch v15 non applicabile (${label}). Ricarica i file Admin dalla versione attesa.`);
  regex.lastIndex = 0;
...
async function loadSource(name) {
  ...
  url.searchParams.set('v15src', VERSION);
...
function draftKey(day){return \`cral-admin-draft-v15|${'${state.target}'}|${'${state.tournament}'}|${'${day}'}\`}
function saveLocalDayDraft()`,
...
`;
  ...
  s += "\n\n/* CRAL_ADMIN_V14_AUTOGOAL_UI\n   L'oggetto storico ownGoals usa beneficiary (squadra che riceve il goal).\n   Per l'utente e' molto piu' naturale selezionare la squadra che ha commesso\n   l'autorete. Qui invertiamo solo la rappresentazione del primo select e\n   popoliamo il secondo select con i giocatori della squadra autrice.\n   Il formato CSV resta compatibile con tutto lo storico. */\nfunction cralV14OtherTeam(match,team){\n  const key=norm(team);\n  if(key===norm(match.home))return match.away;\n  if(key===norm(match.away))return match.home;\n  return '';\n}\nfunction cralV14TextKey(value){return norm(String(value||'').replace(/^[^A-Za-zÀ-ÿ0-9]+/,'').trim())}\nfunction cralV14MatchCard(match){\n  const titleKey=norm(match.home+' vs '+match.away);\n  const candidates=[...document.querySelectorAll('h1,h2,h3,h4,h5,strong,b,.card-title,.match-title')];\n  const title=candidates.find(node=>norm(node.textContent||'')===titleKey);\n  if(!title)return null;\n  let card=title.parentElement;\n  while(card&&card!==document.body){\n    const txt=norm(card.textContent||'');\n    if(txt.includes('autogoal')&&txt.includes('premi'))return card;\n    card=card.parentElement;\n  }\n  return title.parentElement;\n}\nfunction cralV14SectionMarker(card,key){\n  const nodes=[...card.querySelectorAll('h3,h4,h5,strong,b,span,div')].filter(node=>node.tagName!=='BUTTON'&&!node.querySelector('select,input,textarea'));\n  return nodes.find(node=>cralV14TextKey(node.textContent)===key)||null;\n}\nfunction cralV14OwnGoalTeamSelects(card,match,top,bottom){\n  return [...card.querySelectorAll('select')].filter(sel=>{\n    const rect=sel.getBoundingClientRect();\n    if(rect.top+rect.height/2<=top||rect.top>=bottom)return false;\n    const vals=[...sel.options].map(o=>norm(o.value||o.textContent));\n    return vals.includes(norm(match.home))&&vals.includes(norm(match.away));\n  });\n}\nfunction cralV14SmallestRow(selectEl,card){\n  let node=selectEl.parentElement,best=selectEl.parentElement;\n  while(node&&node!==card){\n    const count=node.querySelectorAll('select').length;\n    if(count>=2){best=node;break}\n    node=node.parentElement;\n  }\n  return best;\n}\nfunction cralV14PopulateOwnGoalPlayer(playerSelect,match,goal,offenderTeam){\n  if(!playerSelect||!offenderTeam)return;\n  const current=String(goal?.player||playerSelect.value||'').trim();\n  const players=playersForTeam(state.model,offenderTeam)||[];\n  const options=[{value:'',label:'— giocatore facoltativo —'},...players.map(p=>({value:p.fullName||p.displayName||'',label:p.fullName||p.displayName||''})).filter(p=>p.value)];\n  if(current&&!options.some(o=>norm(o.value)===norm(current)))options.push({value:current,label:current});\n  const signature=norm(offenderTeam)+'|'+options.map(o=>norm(o.value)).join('|');\n  if(playerSelect.dataset.cralV14Options!==signature){\n    playerSelect.innerHTML='';\n    options.forEach(item=>{const opt=document.createElement('option');opt.value=item.value;opt.textContent=item.label;playerSelect.appendChild(opt)});\n    playerSelect.dataset.cralV14Options=signature;\n  }\n  playerSelect.value=options.some(o=>o.value===current)?current:(options.find(o=>norm(o.value)===norm(current))?.value||'');\n  playerSelect.title='Giocatore che ha commesso l’autogoal (facoltativo)';\n  playerSelect.setAttribute('aria-label','Giocatore che ha commesso l’autogoal, facoltativo');\n}\nfunction cralV14EnhanceOwnGoals(){\n  if(!state?.dayDraft?.matches?.length)return;\n  state.dayDraft.matches.forEach(match=>{\n    const goals=match.details?.ownGoals||[];\n    if(!goals.length)return;\n    const card=cralV14MatchCard(match);if(!card)return;\n    const ag=cralV14SectionMarker(card,'autogoal');if(!ag)return;\n    const premi=cralV14SectionMarker(card,'premi');\n    const top=ag.getBoundingClientRect().bottom-2;\n    const bottom=premi?premi.getBoundingClientRect().top:Infinity;\n    const teamSelects=cralV14OwnGoalTeamSelects(card,match,top,bottom).slice(0,goals.length);\n    teamSelects.forEach((teamSelect,index)=>{\n      const goal=goals[index]||{};\n      // Il valore del select resta la beneficiary storica. Cambia solo l'etichetta:\n      // value=home mostra away, value=away mostra home.\n      [...teamSelect.options].forEach(opt=>{\n        const label=norm(opt.value)===norm(match.home)?match.away:(norm(opt.value)===norm(match.away)?match.home:null);\n        if(label!==null && opt.textContent!==label)opt.textContent=label;\n      });\n      if(!teamSelect.dataset.cralV14Bound){teamSelect.dataset.cralV14Bound='1';teamSelect.addEventListener('change',cralV14ScheduleOwnGoalEnhance)}\n      const beneficiary=teamSelect.value||goal.beneficiary||'';\n      const offender=cralV14OtherTeam(match,beneficiary);\n      teamSelect.title='Squadra che ha commesso l’autogoal';\n      teamSelect.setAttribute('aria-label','Squadra che ha commesso l’autogoal');\n      const row=cralV14SmallestRow(teamSelect,card);\n      const selects=[...row.querySelectorAll('select')];\n      const playerSelect=selects.find(x=>x!==teamSelect);\n      cralV14PopulateOwnGoalPlayer(playerSelect,match,goal,offender);\n      if(!row.dataset.cralV14AutogoalHint){\n        row.dataset.cralV14AutogoalHint='1';\n        const hint=document.createElement('div');hint.className='file-meta cral-autogoal-help';\n        hint.textContent='Squadra che ha commesso l’autorete · giocatore facoltativo. Il goal viene attribuito automaticamente all’avversaria.';\n        hint.style.gridColumn='1 / -1';hint.style.marginTop='4px';\n        row.appendChild(hint);\n      }\n    });\n  });\n}\nlet cralV14AutogoalTimer=0;\nfunction cralV14ScheduleOwnGoalEnhance(){\n  clearTimeout(cralV14AutogoalTimer);\n  cralV14AutogoalTimer=setTimeout(()=>{try{cralV14EnhanceOwnGoals()}catch(error){console.warn('[CRAL Admin v15] enhancer autogoal',error)}},0);\n}\nnew MutationObserver(cralV14ScheduleOwnGoalEnhance).observe(document.getElementById('app')||document.body,{subtree:true,childList:true});\nwindow.addEventListener('resize',cralV14ScheduleOwnGoalEnhance,{passive:true});\ncralV14ScheduleOwnGoalEnhance();\n";

  s += "\n\n/* CRAL_ADMIN_V15_AUTOGOAL_UI\n   Second pass robusto: non usa coordinate/layout. Identifica la sezione Autogoal\n   per ordine DOM e mostra sempre la squadra che HA COMMESSO l'autorete. */\nfunction cralV15OtherTeam(match,team){\n  const key=norm(team);if(key===norm(match.home))return match.away;if(key===norm(match.away))return match.home;return '';\n}\nfunction cralV15OwnText(node){\n  return norm([...node.childNodes].filter(n=>n.nodeType===Node.TEXT_NODE).map(n=>n.textContent||'').join(' ').replace(/^[^A-Za-zÀ-ÿ0-9]+/,'').trim());\n}\nfunction cralV15FindCard(match){\n  const hk=norm(match.home),ak=norm(match.away);\n  const heads=[...document.querySelectorAll('h1,h2,h3,h4,h5,strong,b,.card-title,.match-title')];\n  const title=heads.find(n=>{const t=norm(n.textContent||'');return t.includes(hk)&&t.includes(ak)});\n  if(title){let p=title;while(p&&p!==document.body){const t=norm(p.textContent||'');if(t.includes('autogoal')&&t.includes('premi')&&p.querySelectorAll('select').length>=2)return p;p=p.parentElement}}\n  const blocks=[...document.querySelectorAll('.card,article,section,main>div,#app>div,#app div')];\n  const matches=blocks.filter(p=>{const t=norm(p.textContent||'');return t.includes(hk)&&t.includes(ak)&&t.includes('autogoal')&&t.includes('premi')&&p.querySelectorAll('select').length>=2});\n  return matches.sort((a,b)=>a.querySelectorAll('*').length-b.querySelectorAll('*').length)[0]||null;\n}\nfunction cralV15Marker(card,key){\n  const nodes=[...card.querySelectorAll('h2,h3,h4,h5,strong,b,span,div')].filter(n=>!n.matches('button')&&!n.querySelector('select,input,textarea'));\n  return nodes.find(n=>{const own=cralV15OwnText(n);return own===key||own.endsWith(key)||own.startsWith(key)})||null;\n}\nfunction cralV15IsAfter(a,b){return !!(a.compareDocumentPosition(b)&Node.DOCUMENT_POSITION_FOLLOWING)}\nfunction cralV15Between(node,start,end){return cralV15IsAfter(start,node)&&(!end||cralV15IsAfter(node,end))}\nfunction cralV15AutogoalTeamSelects(card,match,start,end){\n  return [...card.querySelectorAll('select')].filter(sel=>{\n    if(!cralV15Between(sel,start,end))return false;\n    const vals=[...sel.options].map(o=>norm(o.value||o.textContent));\n    return vals.includes(norm(match.home))&&vals.includes(norm(match.away));\n  });\n}\nfunction cralV15RowForSelect(sel,card){\n  let p=sel.parentElement,best=sel.parentElement;\n  while(p&&p!==card){\n    const count=p.querySelectorAll('select').length;\n    if(count>=2){best=p;break}\n    p=p.parentElement;\n  }\n  return best;\n}\nfunction cralV15PopulatePlayer(select,offender,goal){\n  if(!select||!offender)return;\n  const current=String(goal?.player||select.value||'').trim();\n  const roster=playersForTeam(state.model,offender)||[];\n  const items=[{value:'',label:'— giocatore facoltativo —'},...roster.map(p=>({value:p.fullName||p.displayName||'',label:p.fullName||p.displayName||''})).filter(x=>x.value)];\n  if(current&&!items.some(x=>norm(x.value)===norm(current)))items.push({value:current,label:current});\n  const sig=norm(offender)+'|'+items.map(x=>norm(x.value)).join('|');\n  if(select.dataset.cralV15Options!==sig){select.innerHTML='';items.forEach(x=>{const o=document.createElement('option');o.value=x.value;o.textContent=x.label;select.appendChild(o)});select.dataset.cralV15Options=sig}\n  select.value=items.find(x=>x.value===current)?.value||items.find(x=>norm(x.value)===norm(current))?.value||'';\n  select.title='Giocatore che ha commesso l’autogoal (facoltativo)';select.setAttribute('aria-label',select.title);\n}\nfunction cralV15EnhanceOwnGoals(){\n  if(!state?.dayDraft?.matches?.length)return;\n  state.dayDraft.matches.forEach(match=>{\n    const goals=match.details?.ownGoals||[];if(!goals.length)return;\n    const card=cralV15FindCard(match);if(!card)return;\n    const start=cralV15Marker(card,'autogoal');if(!start)return;\n    const end=cralV15Marker(card,'premi');\n    const teamSelects=cralV15AutogoalTeamSelects(card,match,start,end).slice(0,goals.length);\n    teamSelects.forEach((teamSelect,i)=>{\n      const goal=goals[i]||{};\n      [...teamSelect.options].forEach(opt=>{\n        const label=norm(opt.value)===norm(match.home)?match.away:(norm(opt.value)===norm(match.away)?match.home:null);\n        if(label!==null && opt.textContent!==label)opt.textContent=label;\n      });\n      const beneficiary=teamSelect.value||goal.beneficiary||'';\n      const offender=cralV15OtherTeam(match,beneficiary);\n      teamSelect.title='Squadra che ha commesso l’autogoal';teamSelect.setAttribute('aria-label',teamSelect.title);\n      if(!teamSelect.dataset.cralV15Bound){teamSelect.dataset.cralV15Bound='1';teamSelect.addEventListener('change',cralV15ScheduleOwnGoals)}\n      const row=cralV15RowForSelect(teamSelect,card);\n      const playerSelect=[...row.querySelectorAll('select')].find(x=>x!==teamSelect);\n      cralV15PopulatePlayer(playerSelect,offender,goal);\n      if(!row.dataset.cralV15Hint){row.dataset.cralV15Hint='1';const hint=document.createElement('div');hint.className='file-meta cral-autogoal-help';hint.textContent='Squadra che ha commesso l’autorete · giocatore facoltativo. Il goal viene attribuito automaticamente all’avversaria.';hint.style.gridColumn='1 / -1';hint.style.marginTop='4px';row.appendChild(hint)}\n    });\n  });\n}\nlet cralV15OwnGoalTimer=0;\nfunction cralV15ScheduleOwnGoals(){clearTimeout(cralV15OwnGoalTimer);cralV15OwnGoalTimer=setTimeout(()=>{try{cralV15EnhanceOwnGoals()}catch(error){console.warn('[CRAL Admin v15] autogoal UI',error)}},30)}\nnew MutationObserver(cralV15ScheduleOwnGoals).observe(document.getElementById('app')||document.body,{subtree:true,childList:true});\ndocument.addEventListener('change',cralV15ScheduleOwnGoals,true);\ncralV15ScheduleOwnGoals();\n";
...
async function boot() {
  try {
    ...
    const coreUrl = moduleUrl(patchCore(coreOriginal), 'cral-core-v15.js');
    const ghUrl = moduleUrl(ghPatched, 'cral-gh-v15.js');
    const adminUrl = moduleUrl(patchAdmin(adminOriginal, coreUrl, ghUrl), 'cral-admin-v15.js');

    await import(adminUrl);
  } catch (error) {
    console.error('[CRAL Admin v15]', error);
    ...
    if (app) {
      ...
      const title = document.createElement('h2'); title.textContent = 'Errore bootstrap Admin v15';
