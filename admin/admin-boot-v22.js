...
function replaceOnce(source, needle, replacement, label) {
  ...
  if (index < 0) throw new Error(`Patch v22 non applicabile (${label}). Ricarica i file Admin dalla versione attesa.`);
  return source.slice(0, index) + replacement + source.slice(index + needle.length);
}

function replaceRegexOnce(source, regex, replacement, label) {
  if (!regex.test(source)) throw new Error(`Patch v22 non applicabile (${label}). Ricarica i file Admin dalla versione attesa.`);
  regex.lastIndex = 0;
...
async function loadSource(name) {
  ...
  url.searchParams.set('v22src', VERSION);
...
/* CRAL_ADMIN_V22_FANTA_MANIFEST_GUARD
...
async function cralV22LoadProtectedManifest(){
  ...
  try{
    ...
    console.warn('[CRAL Admin v22] lettura manifest Fantacalcio',error);
  }
...
function cralV22ScheduleManifestGuard(){
  ...
  cralV22ManifestGuardTimer=setTimeout(()=>{cralV22ProtectPendingManifest().catch(error=>console.warn('[CRAL Admin v22] protezione manifest',error));},30);
}
...
function cralV16ScheduleOwnGoals(){
  ...
  cralV16OwnGoalTimer=setTimeout(()=>{try{cralV16EnhanceOwnGoals()}catch(error){console.warn('[CRAL Admin v22] autogoal UI',error)}},20);
}
...
async function boot() {
  try {
    ...
    const coreUrl = moduleUrl(patchCore(coreOriginal), 'cral-core-v22.js');
    const ghUrl = moduleUrl(ghPatched, 'cral-gh-v22.js');
    const adminUrl = moduleUrl(patchAdmin(adminOriginal, coreUrl, ghUrl), 'cral-admin-v22.js');

    await import(adminUrl);
  } catch (error) {
    console.error('[CRAL Admin v22]', error);
    ...
    if (app) {
      ...
      const title = document.createElement('h2'); title.textContent = 'Errore bootstrap Admin v22';
