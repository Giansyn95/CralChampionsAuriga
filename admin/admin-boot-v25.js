/*
 * CRAL Champions Admin - bootstrap logica v22
 *
...
function replaceOnce(source, needle, replacement, label) {
  ...
  if (index < 0) throw new Error(`Patch v25 non applicabile (${label}). Ricarica i file Admin dalla versione attesa.`);
  return source.slice(0, index) + replacement + source.slice(index + needle.length);
}

function replaceRegexOnce(source, regex, replacement, label) {
  if (!regex.test(source)) throw new Error(`Patch v25 non applicabile (${label}). Ricarica i file Admin dalla versione attesa.`);
  regex.lastIndex = 0;
...
async function loadSource(name) {
  ...
  url.searchParams.set('v25src', VERSION);
...
function draftKey(day){return \`cral-admin-draft-v16|${'${state.target}'}|${'${state.tournament}'}|${'${day}'}\`}
function saveLocalDayDraft()`,
...
/* CRAL_ADMIN_V25_FANTA_MANIFEST_GUARD
...
async function cralV22LoadProtectedManifest(){
  ...
  try{
    ...
    console.warn('[CRAL Admin v25] lettura manifest Fantacalcio',error);
  }
...
function cralV22ScheduleManifestGuard(){
  ...
  cralV22ManifestGuardTimer=setTimeout(()=>{cralV22ProtectPendingManifest().catch(error=>console.warn('[CRAL Admin v25] protezione manifest',error));},30);
}
...
function cralV16ScheduleOwnGoals(){
  ...
  cralV16OwnGoalTimer=setTimeout(()=>{try{cralV16EnhanceOwnGoals()}catch(error){console.warn('[CRAL Admin v25] autogoal UI',error)}},20);
}
...
async function boot() {
  try {
    ...
    const coreUrl = moduleUrl(patchCore(coreOriginal), 'cral-core-v25.js');
    const ghUrl = moduleUrl(ghPatched, 'cral-gh-v25.js');
    const adminUrl = moduleUrl(patchAdmin(adminOriginal, coreUrl, ghUrl), 'cral-admin-v25.js');

    await import(adminUrl);
  } catch (error) {
    console.error('[CRAL Admin v25]', error);
    ...
    if (app) {
      ...
      const title = document.createElement('h2'); title.textContent = 'Errore bootstrap Admin v25';
