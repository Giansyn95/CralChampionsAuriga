/*
 * CRAL Champions Admin - bootstrap logica v17
 *
...
function replaceOnce(source, needle, replacement, label) {
  ...
  if (index < 0) throw new Error(`Patch v17 non applicabile (${label}). Ricarica i file Admin dalla versione attesa.`);
  return source.slice(0, index) + replacement + source.slice(index + needle.length);
}

function replaceRegexOnce(source, regex, replacement, label) {
  if (!regex.test(source)) throw new Error(`Patch v17 non applicabile (${label}). Ricarica i file Admin dalla versione attesa.`);
  regex.lastIndex = 0;
...
async function loadSource(name) {
  ...
  url.searchParams.set('v17src', VERSION);
...
function cralV16ScheduleOwnGoals(){
  ...
  cralV16OwnGoalTimer=setTimeout(()=>{try{cralV16EnhanceOwnGoals()}catch(error){console.warn('[CRAL Admin v17] autogoal UI',error)}},20);
}
...
async function boot() {
  try {
    ...
    const coreUrl = moduleUrl(patchCore(coreOriginal), 'cral-core-v17.js');
    const ghUrl = moduleUrl(ghPatched, 'cral-gh-v17.js');
    const adminUrl = moduleUrl(patchAdmin(adminOriginal, coreUrl, ghUrl), 'cral-admin-v17.js');

    await import(adminUrl);
  } catch (error) {
    console.error('[CRAL Admin v17]', error);
    ...
    if (app) {
      ...
      const title = document.createElement('h2'); title.textContent = 'Errore bootstrap Admin v17';
