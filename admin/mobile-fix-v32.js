/*
 * CRAL Champions Admin - iOS/mobile viewport fix v32
 *
 ...
 *
 * V32 esegue il reset verticale SOLO nella transizione LOGIN -> ADMIN e
 * mantiene il top per qualche istante mentre Safari chiude autofill/tastiera e
 * ristabilisce il visual viewport.
...
(() => {
  ...
  function clearLoginTopTimers() {
    loginTopTimers.forEach(clearTimeout);
    loginTopTimers = [];
  }

  /*
   * Safari/iOS può modificare ancora il visual viewport dopo che il DOM login
   * è già stato sostituito, soprattutto con token riempito da Passwords/AutoFill.
   * Il reset è volutamente one-shot: viene eseguito solo subito dopo il login,
   * mai mentre si compila o si naviga nell'Admin.
   */
  function pinAdminToTopAfterLogin() {
    clearLoginTopTimers();
    const reset = () => {
    ...
    reset();
    [60, 140, 280, 520, 850, 1200].forEach(ms => {
      loginTopTimers.push(setTimeout(reset, ms));
    });
  }
  ...
  function inspectApp() {
    ...
    if (nextMode === 'admin') {
      ...
      if (previousMode !== 'admin') {
        ...
        if (loginExitPending || previousMode === 'login' || previousMode === 'other') {
          pinAdminToTopAfterLogin();
